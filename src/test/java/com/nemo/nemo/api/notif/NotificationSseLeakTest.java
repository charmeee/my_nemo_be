...
class NotificationSseLeakTest extends ApiE2ETestBase {
    ...
    void sseSubscribe_doesNotExhaustHikariPool() throws Exception {
        ...
        // SSE 는 long-running 응답이라 HttpClient.close() (graceful) 로는 안 끝남.
        // 테스트 종료 시 shutdownNow() 로 즉시 cancel → 서버 SseEmitter onError 발동
        // → emitter cleanup. 안 그러면 SSE_TIMEOUT(30분) 동안 살아 있어 후속 테스트의
        // 컨텍스트 종료에 영향(다음 테스트 컨텍스트가 닫힌 채로 요청을 받아 500).
        ...
        try {
            ...
            try {
                ...
                // 비동기 응답 안정화 (= 실제로 leak 이 발생하는 시점) 후 풀 상태 측정
                Thread.sleep(1500);
        ...
        // 서버가 onError → cleanupEmitter 처리할 여유
        Thread.sleep(500);
    }
